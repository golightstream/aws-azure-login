...
module.exports = {
    async loginAsync(profileName, mode, disableSandbox, noPrompt, enableChromeNetworkService, awsNoVerifySsl, enableChromeSeamlessSso) {
        ...
        const account_map = this._parseAccountMappings(profile);
        ...
        const { role, durationHours } = await this._askUserForRoleAndDurationAsync(roles, noPrompt, profile.azure_default_role_arn, profile.azure_default_duration_hours, account_map);
        await this._assumeRoleAsync(profileName, samlResponse, role, durationHours, awsNoVerifySsl);
    },

    /**
     * Takes the profile data and looks for all keys
     * starting with 'map_' and returns a dict with those
     * items for use in safe-name mapping while choosing
     * from multiple roles
     * @param {string} profile - The profile object
     * @returns {string} Object of ID to Name maps
     * @private
     */
    _parseAccountMappings(profile) {
        var mappings = {};
        for ( let key of Object.keys(profile) ) {
            if ( key.startsWith('map_') ) {
                let safe_key = key.replace('map_', '');
                debug(`Mapping: ${safe_key}=${profile[key]}`);
                mappings[safe_key] = profile[key];
            }
        }
        debug(mappings);
        return mappings;
     ...
     * @param {<{}>} [accountMap] - The optional account ID to name map in .aws/config
     * @returns {Promise.<{role: string, durationHours: number}>} The selected role and duration
     * @private
     */
    async _askUserForRoleAndDurationAsync(roles, noPrompt, defaultRoleArn, defaultDurationHours, accountMap) {
        let role;
        let durationHours;
        const questions = [];
        // check to see if we can add a "safeName" option to each role
        roles.forEach((item, index) => {
            // split the ARN to get the ID
            let arnSplit = item.roleArn.split(":");
            let accountId = arnSplit['4'];
            item.safeName = accountMap.hasOwnProperty(accountId) ? `${accountMap[accountId]}:${arnSplit[5]}` : item.roleArn;
            roles[index] = item;
        });
        if (roles.length === 0) {
            ...
            if (role) {
                ...
                questions.push({
                    ...
                    message: "Role:",
                    type: "list",
                    choices: _.sortBy(_.map(roles, "safeName")),
        ...
        if (!role) role = _.find(roles, ["safeName", answers.role]);
